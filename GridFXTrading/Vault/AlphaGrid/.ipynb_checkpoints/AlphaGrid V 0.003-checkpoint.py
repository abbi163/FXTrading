  ...
  { 
   ...
   if (IsNewCandle())
      ...
      {  GridOpen();
         ...
         Comment(StringFormat("Last Order Open price :  %G",lastOrderType));
         // --------------------------------------------------
      } 
...
// ------------------- TODO ---------------------------------
// GridOpen Is not yet working !! Fix this !!
void GridOpen()
{
    int lastOrderType = getLastOrderType() ;
    double lastOpenPrice = getLastOrderOpenPrice() ;

    if(lastOrderType == 0 && lastOpenPrice - Ask > gGridStep)
        {
            int buyTicket = Trade.BuyNow();
        }

    if(lastOrderType == 1 && Bid - lastOpenPrice > gGridStep)
        {
            int sellTicket = Trade.SellNow();
        }
}